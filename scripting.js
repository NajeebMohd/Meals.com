...
function fetchImg(Img,detail,button){
    ...
    request.onload = function(){
       ...
       console.log(request);
       console.log(json);// for video explanation
        // for the detail page
...
navButton.addEventListener('click',function(){
    ...
    if(!barclick){
        hiddenDiv.style.display = 'block'; // opening the hidden div
        barclick = true;
    }else{
        hiddenDiv.style.display = 'none';// closing th hidden div
        barclick = false;
    }  
...
btn2.addEventListener('click',function(){    // find meals button
    if(search2.value == ''){                 // search in the header
        window.alert('Fill the value in the search');
    }else{                  
        var child = container.lastElementChild; 
        while (child) {
            container.removeChild(child);   // removing the divs
        ...
        function searchFun(){
            ...
            request.onload = function(){                
                ...
                console.log(Meals);                
                if(Meals == null) window.alert('No Results for '+search2.value);
